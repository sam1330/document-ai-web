...
import { ArrowDownTrayIcon, PencilSquareIcon, SparklesIcon, TrashIcon } from '@heroicons/react/24/outline'
...
export default function ResumeBuilder({ resumeId }: ResumeBuilderProps) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    const toastId = toast.loading(t('resumes.delete.progress') || 'Deleting...')
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await api.post(`/api/resumes/${resumeId}/generate`, {}, { responseType: 'blob' })

      const blob = new Blob([response.data], { type: 'application/pdf' })
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', `haku-resume-${data.cv.name.replace(/\s+/g, '-').toLowerCase()}.pdf`)
      ...
      <div className="h-16 border-b border-slate-200/60 px-4 md:px-6 flex items-center justify-between bg-white/80 backdrop-blur-md z-20">
        ...
        <div className="flex items-center space-x-2 md:space-x-3">
          {resumeId && (
            ...
            >
              <span className="hidden sm:inline">{t('resumes.actions.delete') || 'Delete'}</span>
            </Button>
          )}
          <Button
            size="sm"
          ...
          >
            <span className="hidden sm:inline">{t('resumes.builder.downloadPDF')}</span>
        ...
        <main className={cn(
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur shadow-2xl border border-slate-200 px-6 py-3 rounded-full flex items-center space-x-6 z-30">
            <div className="flex items-center space-x-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('resumes.builder.template')}</span>
              <span className="text-sm font-bold text-slate-900">{t('resumes.builder.design.templates.classic.name')}</span>
            </div>
            <div className="w-px h-4 bg-slate-200"></div>
            <div className="flex items-center space-x-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('resumes.builder.scale')}</span>
              <span className="text-sm font-bold text-slate-900">{t('resumes.builder.fitToWidth')}</span>
            </div>
          </div>
        </main>
