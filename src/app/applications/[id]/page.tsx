...
export default function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true)
      const appRes = await api.get(`/api/job-application/${id}`)
      ...
      if (appData.resume_id) {
        const resumeRes = await api.get(`/api/resume/${appData.resume_id}`)
      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-slate-50/50">
          ...
          <div className="max-w-7xl mx-auto py-20 px-4 text-center">
             ...
             <Link href="/applications" className="text-indigo-600 font-bold mt-4 inline-block hover:underline">
  ...
  const statusMap = {
    draft: { icon: PencilIcon, color: 'text-slate-500', bg: 'bg-slate-50', border: 'border-slate-100' },
    applied: { icon: BriefcaseIcon, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100' },
    interview: { icon: ClockIcon, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100' },
    accepted: { icon: CheckBadgeIcon, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' },
    rejected: { icon: XCircleIcon, color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' },
  }
      ...
      <div className="min-h-screen bg-slate-50/50 pb-12">
        ...
        <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
           ...
           <div className="flex items-center space-x-2 text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">
              <Link href="/applications" className="hover:text-indigo-600 transition-colors">Applications</Link>
           ...
           >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                 <div className="flex items-start space-x-6">
                    ...
                    <div className="pt-1">
                       <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">{application.position_title}</h1>
                       <div className="flex flex-wrap items-center gap-4 mt-2">
                          <p className="text-lg font-bold text-slate-600">{application.company_name}</p>
                          <div className={`flex items-center space-x-2 px-4 py-1.5 rounded-full border ${currentStatus.bg} ${currentStatus.border} ${currentStatus.color}`}>
                             <currentStatus.icon className="h-4 w-4" />
                             <span className="text-xs font-black uppercase tracking-widest">{application.status}</span>
                          </div>
                       </div>
                    </div>
                 </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 >
                    <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                       <div className="flex items-center space-x-3">
                          ...
                          <h2 className="font-black text-slate-800 uppercase text-xs tracking-widest">Job Description</h2>
                       </div>
                 ...
                 >
                    <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                       <div className="flex items-center space-x-3">
                          ...
                          <h2 className="font-black text-slate-800 uppercase text-xs tracking-widest">Personal Notes</h2>
                       </div>
                       {!isEditingNotes && (
                         <button 
              ...
              <div className="space-y-8">
                 ...
                 >
                    <h2 className="font-black text-slate-800 uppercase text-xs tracking-widest mb-6">AI Match Analysis</h2>
                    <div className="flex flex-col items-center">
                       <div className="relative h-32 w-32 flex items-center justify-center mb-4">
                          <svg className="h-full w-full transform -rotate-90">
                             ...
                             <circle 
                               ...
                               strokeDasharray={364}
                               strokeDashoffset={364 - (364 * (latestAnalysis?.analysis_results?.atsScore || 0)) / 100}
                       ...
                       <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">ATS Compatibility Score</p>
                       {latestAnalysis ? (
                         <Link 
                         ...
                         <p className="text-[10px] text-slate-400 text-center italic">Register more data for high-fidelity scoring.</p>
                       )}
                    </div>
                 </motion.div>
                 ...
                 <motion.div 
                   ...
                   className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm"
                 >
                    <h2 className="font-black text-slate-800 uppercase text-xs tracking-widest mb-6">Pipeline Stage</h2>
                    <div className="space-y-3">
                       {Object.keys(statusMap).map((s) => {
                          ...
                          return (
                             ...
                             >
                                <div className="flex items-center space-x-3">
                                   <sm.icon className={`h-4 w-4 ${isSelected ? 'text-indigo-600' : 'text-slate-400'}`} />
                                   <span className={`text-xs font-black uppercase tracking-widest ${isSelected ? 'text-indigo-900' : 'text-slate-600'}`}>{s}</span>
                                </div>
                 ...
                 <motion.div 
                   ...
                   className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm"
                 >
                    <h2 className="font-black text-slate-800 uppercase text-xs tracking-widest mb-6">Quick Info</h2>
                    <div className="space-y-6">
                       <InfoItem icon={CalendarIcon} label="Creation Date" value={formatDate(application.created_at)} />
                       {application.application_deadline && (
                         <InfoItem icon={ClockIcon} label="Deadline" value={formatDate(application.application_deadline)} />
                       )}
                       <InfoItem icon={LinkIcon} label="Job URL" value={
                         application.application_url ? (
                           <a href={application.application_url} target="_blank" className="text-indigo-600 hover:underline">Link &rarr;</a>
                         ) : 'Not specified'
                       } />
                       <div className="pt-4 border-t border-slate-50">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Linked Assets</p>
                          <Link 
                            href={`/resumes/${application.resume_id}`}
                          ...
                          >
                             ...
                             <div className="min-w-0">
                                <p className="text-xs font-bold text-slate-600 truncate">{resume?.original_filename || 'Active Resume'}</p>
                             </div>
                          </Link>
                       </div>
                    </div>
